...
% Modified to allow closed path ROI. --Jiacheng Wang, 2/4/2024
...
%In the new version, this function is no longer needed.
...
%Modified spline fitting that fits x to total length and y to total length
%separately.
%Jiacheng Wang, 2/4/2024
function [xspl, yspl] = fitsplineL(xval, yval)
dists = sqrt(diff(xval).^2+diff(yval).^2);
dists = [0; dists]; %supplement a zero at the beginning to make dists the same length as xval.
Lt = [];
for i = 1:length(dists)
    L = sum(dists(1:i));
    Lt = [Lt, L];
end
%fit path length to x and y.
ppx = spline(Lt, xval');
ppy = spline(Lt, yval');
%Get x and y coordinates when L are integers.
%Make sure the interpolated points have distances of 1.
total_L = round(Lt(end));
Lspl = 0:1:total_L;
xspl = ppval(ppx, Lspl);
yspl = ppval(ppy, Lspl);
end
